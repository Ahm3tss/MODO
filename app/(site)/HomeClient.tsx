...
interface HomeClientProps {
  testimonial?: { quote: string; author: string; country: string };
  trustStats?: TrustBarStat[];
}

export default function HomeClient({ testimonial: testimonialProp, trustStats }: HomeClientProps) {
  const { scrollYProgress } = useScroll();
  const heroOpacity = useTransform(scrollYProgress, [0, 0.15], [1, 0]);

  const testimonial = testimonialProp ?? {
    quote: c.testimonial.quote,
    author: c.testimonial.author,
    country: c.testimonial.country,
  };
        ...
        <div className="max-w-4xl mx-auto px-6">
          <BlurFadeIn className="text-center">
            ...
            <p className="text-slate-300">
              <span className="text-white font-medium">{testimonial.author}</span> — {testimonial.country}
            </p>
          </BlurFadeIn>
        </div>
      </section>
