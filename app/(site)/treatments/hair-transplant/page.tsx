...
const c = content.treatments.hairTransplant;
        ...
        <div className="min-h-screen bg-[#020410] text-white selection:bg-accent selection:text-white">
            ...
            <section className="min-h-[80vh] flex items-center justify-center relative border-b border-white/5 overflow-hidden">
                <div className="absolute inset-0 z-0 opacity-40">
                    <Image src={c.hero.image} alt="Hair Transplant" fill className="object-cover" priority />
                ...
                <div className="relative z-10 text-center max-w-5xl px-6 space-y-8">
                    <FadeIn>
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/10 border border-accent/20 text-accent font-mono text-xs tracking-widest uppercase mb-6">
                            <Sparkles className="w-3 h-3" /><span>{c.hero.badge}</span>
                        </div>
                        <h1 className="text-[12vw] md:text-[9vw] font-black leading-[0.9] tracking-tighter bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60 mb-6">
                            {c.hero.heading.split("\n").map((line, i) => (
                                <span key={i}>{line}{i === 0 && <br />}</span>
                            ))}
                        </h1>
                        <p className="text-lg md:text-2xl text-slate-300 font-light max-w-2xl mx-auto mb-8">{c.hero.subtitle}</p>
                        ...
                        <div className="flex flex-col md:flex-row gap-16 items-start">
                            ...
                            <div className="md:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-white/[0.02] backdrop-blur-xl border border-white/[0.05] rounded-2xl p-8">
                                    <h3 className="text-xl font-bold mb-4 text-white flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-accent" /> {c.candidates.idealFor.title}</h3>
                                    <ul className="space-y-3 text-slate-300">
                                        {c.candidates.idealFor.items.map((item) => (
                                            <li key={item} className="flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-accent" />{item}</li>
                                        ))}
                                    </ul>
                                </div>
                                <div className="bg-white/[0.02] backdrop-blur-xl border border-white/[0.05] rounded-2xl p-8">
                                    <h3 className="text-xl font-bold mb-4 text-white">{c.candidates.consultRequired.title}</h3>
                                    <ul className="space-y-3 text-slate-300">
                                        {c.candidates.consultRequired.items.map((item) => (
                                            <li key={item} className="flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-white/30" />{item}</li>
                                        ))}
                                    </ul>
                                </div>
                            </div>
